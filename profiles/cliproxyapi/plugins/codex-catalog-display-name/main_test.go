...
func TestHandleMethod(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "register declares only response interceptor", method: "plugin.register", request: []byte(`{"schema_version":6}`), wantOK: true, wantBody: `"response_interceptor":true`},
		...
		{name: "multiple scopes preserve numbers and unknown fields", method: "response.intercept_after", request: request(catalog, "openai", 200, false), wantOK: true, wantBody: `"display_name":"GPT-5 · team-a"`},
		{name: "balanced model remains unchanged", method: "response.intercept_after", request: request(`{"models":[{"slug":"codex-gpt-5","display_name":"GPT-5"},{"slug":"other/model","display_name":"Other"}]}`, "openai", 200, false), wantOK: true, wantEmptyBody: true},
		{name: "suffix is idempotent", method: "response.intercept_after", request: request(`{"models":[{"slug":"codex-team/gpt-5","display_name":"GPT-5 · team"}]}`, "openai", 200, false), wantOK: true, wantEmptyBody: true},
...
func TestResponseRequestUsesGoJSONByteAndHeaderSemantics(t *testing.T) {
	...
	if !contains(string(response.Body), `"display_name":"Model · scope"`) {
