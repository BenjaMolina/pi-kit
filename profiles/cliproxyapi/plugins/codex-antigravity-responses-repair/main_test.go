...
func TestRequestNormalization(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "claude on antigravity is repaired", request: request(body([]string{identityOne}, "ordinary user text"), "openai-response", "antigravity", "claude-opus-4-6-thinking"), wantChanged: true, wantFragments: []string{"You are Codex; an AI coding agent based on GPT-5."}},
